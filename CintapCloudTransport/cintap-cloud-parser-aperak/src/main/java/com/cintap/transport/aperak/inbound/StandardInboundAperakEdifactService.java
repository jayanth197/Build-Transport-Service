...
import com.cintap.transport.aperak.edifact.parser.AperakEdifactToXmlParser;
...
@Slf4j
@Service("StandardInboundAperakEdifactService")
public class StandardInboundAperakEdifactService implements StandardInboundService {

	@Autowired
	AperakEdifactToXmlParser aperakInboundEdifactParser;
	...
	@Override
	public FileUploadParams processInboundRequest(String interchanges, FileUploadParams fileUploadParams) {

		log.info("StandardInboundOrdersEdiFactService : Request received");
		try {
			// Map data from interchange to Database entity
			fileUploadParams = aperakInboundEdifactParser.convertModelToEntity(interchanges, fileUploadParams);
			...
			TransactionLogInboundOutbound transactionLogInboundOutbound = TransactionLogInboundOutbound.builder()
					...
					.ackRefNumber(fileUploadParams.getAckRefNum()).receivedRawFile(fileUploadParams.getRawFile())
