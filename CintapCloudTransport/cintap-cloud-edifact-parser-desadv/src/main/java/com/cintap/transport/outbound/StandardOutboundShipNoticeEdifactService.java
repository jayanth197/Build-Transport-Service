...
import com.cintap.transport.parser.ShipNoticeOutboundEdifactParser;
...
@Slf4j
@Service("StandardOutboundShipNoticeEdifactService")
public class StandardOutboundShipNoticeEdifactService implements StandardOutboundService {
	...
	@Autowired
	ShipNoticeOutboundEdifactParser shipNoticeOutboundEdifactParser;
		...
		try {
			...
			if (optDespatchAdviceShipNoticeHeader.isPresent()) {
				...
				if (optTransactionLog.isPresent()) {
					...
					edifactData = shipNoticeOutboundEdifactParser.generateOutbound(despatchAdviceShipNoticeHeader,
					...
					auditLogService.saveAuditLog(fileUploadParams.getSenderPartnerId(), bpiLogId,
							...
							"DESADV PACK OUTBOUND EDIFACT generated successfully # " + bpiLogId);
				}
			}
		} catch (Exception e) {
