...
@RestController
@RequestMapping("/v1")
public class ShipNoticeController {
	...
	@Autowired
	DespatchAdviceShipNoticeHeaderRepository shipNoticeHeaderRepository;
	...
	@GetMapping("/shipNotice/{bpiLogId}")
	public ResponseEntity<DespatchAdviceShipNoticeHeader> getDesadvAsnWithBpiLogId(@PathVariable("bpiLogId") Integer bpiLogId) {
		log.info("Sending req to orders ");

		DespatchAdviceShipNoticeHeader despatchAdviceShipNoticeHeader = null;
		try {
			Optional<DespatchAdviceShipNoticeHeader> optDespatchAdviceShipNoticeHeader = shipNoticeHeaderRepository.findByBpiLogId(bpiLogId);
			if (optDespatchAdviceShipNoticeHeader.isPresent()) {
				despatchAdviceShipNoticeHeader = optDespatchAdviceShipNoticeHeader.get();
			}
		} catch (Exception e) {
		...
		return new ResponseEntity<>(despatchAdviceShipNoticeHeader, HttpStatus.OK);
	}
}
